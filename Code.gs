...
function sheetToRows(sh, mapper) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    rows.push(mapper(data[i], i + 1)); // rowIndex is 1-based
  }
  return rows;
}
...
// ── ACCOUNTS (with live balance computation) ──────────────────
...
function getCategories() {
  ...
  var cats = sheetToRows(sh, function(d, ri){ return { rowIndex: ri, name: d[0] }; });
...
function getAllTransactions(p) {
  ...
  sheetToRows(expSh, function(d, ri) {
    return { rowIndex: ri, type: "expense", date: formatDate(d[0]), amount: toFloat(d[1]), category: d[2], note: d[3], account: d[4] };
  }).forEach(function(r){ if (inPeriod(r.date)) txns.push(r); });

  sheetToRows(incSh, function(d, ri) {
    return { rowIndex: ri, type: "income", date: formatDate(d[0]), amount: toFloat(d[1]), category: d[2], note: d[3], account: d[4] };
  }).forEach(function(r){ if (inPeriod(r.date)) txns.push(r); });

  sheetToRows(trfSh, function(d, ri) {
    return { rowIndex: ri, type: "transfer", date: formatDate(d[0]), amount: toFloat(d[1]), fromAccount: d[2], toAccount: d[3], note: d[4] };
  }).forEach(function(r){ if (inPeriod(r.date)) txns.push(r); });
...
function getSummary(p) {
  ...
  sheetToRows(expSh, function(d){ return { date: formatDate(d[0]), amount: toFloat(d[1]) }; })
    .forEach(function(r){ if (inMonth(r.date)) totalExpense += r.amount; });
  sheetToRows(incSh, function(d){ return { date: formatDate(d[0]), amount: toFloat(d[1]) }; })
    .forEach(function(r){ if (inMonth(r.date)) totalIncome += r.amount; });

  return jsonResponse({ ok: true, totalExpense: Math.round(totalExpense*100)/100, totalIncome: Math.round(totalIncome*100)/100, net: Math.round((totalIncome-totalExpense)*100)/100 });
}
